...
export function computeBudgetSummary(owners = [], records = []) {
  ...
  [...ownerSummaries, orphanSummary].forEach((bucket) => {
    bucket.netAvailable = Number((bucket.grossIncome - bucket.expenseTotal - bucket.reserveTotal).toFixed(2));
  ...
  const familyNetAvailable = Number((familyGrossIncome - familyExpenseTotal - familyReserveTotal).toFixed(2));
