  ...
  async fetchOpenFinanceConnections(userId) {
    ...
    snapshot.forEach((doc) => {
      ...
      connections.push({
        ...
        provider: String(data.provider || 'meu-pluggy').trim(),
        providerConnectionId: String(data.providerConnectionId || '').trim(),
        providerItemId: String(data.providerItemId || '').trim(),
        status: String(data.status || 'unknown').trim(),
        consentUrl: String(data.consentUrl || '').trim(),
        ...
        lastWebhookEvent: String(data.lastWebhookEvent || '').trim(),
        lastWebhookAt: String(data.lastWebhookAt || '').trim(),
        itemStatusRaw: String(data.itemStatusRaw || '').trim(),
        connectorId: String(data.connectorId || '').trim(),
        connectorName: String(data.connectorName || '').trim(),
